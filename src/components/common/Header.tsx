...
export const Header: React.FC = () => {
  ...
  return (
    <header className={`h-14 border-b px-5 flex items-center justify-between z-30 shrink-0 sticky top-0 transition-colors backdrop-blur-md ${
      ...
      {/* Zone 2: Global Search Affordance (Cmd+K) */}
      <div className="flex-1 max-w-md mx-4 hidden md:block">
        <button
          onClick={() => setIsCommandPaletteOpen(true)}
          className={`w-full flex items-center justify-between px-3 py-1.5 border rounded-lg text-xs transition-colors ${
            isDark 
        ...
        >
          <div className="flex items-center gap-2">
            <Search className={`w-3.5 h-3.5 ${isDark ? 'text-neutral-400' : 'text-neutral-400'}`} />
            <span className="font-normal text-xs">Поиск или команда <span className="font-mono text-[11px] text-[#2563EB]">/new-deal, /add-task...</span></span>
          </div>
          <kbd className={`px-1.5 py-0.5 text-[10px] font-mono rounded border ${
            isDark 
              ? 'bg-neutral-800 text-neutral-300 border-neutral-700' 
              : 'bg-[#F8F7F4] text-neutral-600 border-black/[0.08]'
          }`}>
            ⌘K
          </kbd>
        </button>
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-[#2563EB] hover:bg-[#1D4ED8] text-white rounded-lg text-xs font-semibold transition-colors shadow-2xs"
          >
          ...
          {isNewMenuOpen && (
            <>
              ...
              />
              <div className={`absolute right-0 mt-1.5 w-52 border rounded-xl shadow-xl py-1.5 z-50 text-xs font-medium ${
                isDark 
                  ? 'bg-[#181818] border-white/[0.08] text-neutral-200' 
                  : 'bg-white border-black/[0.08] text-[#1A1A1A]'
              }`}>
                <button
                  onClick={() => { setIsNewMenuOpen(false); setIsCreateClientOpen(true); }}
                ...
                <button
                  onClick={() => { setIsNewMenuOpen(false); setIsCreateDealOpen(true); }}
                  className={`w-full flex items-center gap-2.5 px-3 py-2 text-left transition-colors ${
                    isDark ? 'hover:bg-slate-800 text-slate-200' : 'hover:bg-slate-50 text-slate-800'
                  }`}
                >
                ...
                <button
                  onClick={() => { setIsNewMenuOpen(false); setIsCreateTaskOpen(true); }}
                  className={`w-full flex items-center gap-2.5 px-3 py-2 text-left transition-colors ${
                    isDark ? 'hover:bg-slate-800 text-slate-200' : 'hover:bg-slate-50 text-slate-800'
                  }`}
                >
                ...
                <button
                  onClick={() => { setIsNewMenuOpen(false); setIsCreateInvoiceOpen(true); }}
                  className={`w-full flex items-center gap-2.5 px-3 py-2 text-left transition-colors ${
                    isDark ? 'hover:bg-slate-800 text-slate-200' : 'hover:bg-slate-50 text-slate-800'
                  }`}
                >
                  <FileText className="w-4 h-4 text-purple-500" />
                  <span>Выставить КП / Счет</span>
                </button>
              </div>
            </>
          )}
        ...
        <button
          ...
          className={`relative p-2 rounded-lg border transition-colors ${
            isDark 
              ? 'border-slate-800 hover:border-slate-700 text-slate-300 hover:text-white hover:bg-slate-800' 
        ...
        <div className={`relative pl-1 border-l ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
          ...
          {isManagerMenuOpen && (
            <>
              <div 
                className="fixed inset-0 z-40" 
                onClick={() => setIsManagerMenuOpen(false)} 
              />
              <div className={`absolute right-0 mt-1.5 w-60 border rounded-lg shadow-xl py-1 z-50 text-xs ${
                isDark 
                ...
                {managers.map(m => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-3 py-2 text-left transition-colors ${
                      currentManager.id === m.id 
                        ? (isDark ? 'bg-indigo-950 text-indigo-200' : 'bg-indigo-50 text-indigo-900 font-semibold')
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className={`text-[10px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{m.role}</div>
                    </div>
                  </button>
                ))}
              </div>
